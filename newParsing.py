...
link = "https://polimi365-my.sharepoint.com/:x:/g/personal/10713791_polimi_it/EVNC5KZIcI9ChhNovdvUVkgBdI3d0AaHmvQXYHXUq2Dv_A?e=hGwutN"
# Defining vars
listEventi = []
# insert the time range
Orario = ["8,45-9,45", "9,45-10,45", "10,45-11,45", "11,45-12,45", "14,15-15,15", "15,15-16,15", "16,15-17,15"]
            ...
            if (i != 0 and i < 35):  # i need to skip the first blank row and
                if (sheet.cell_value(i, currentColumnMonth) != ""):  # check the value if it is not null
                    ...
                    for n in range(0, hourNumber):  # number of hours of lessons
                        ...
                        startDateTime = datetime.datetime(int(completeData[0]), int(completeData[1]),
                                                          int(completeData[2]), int(startTime[0]), int(startTime[1]),
                                                          0).strftime("%Y-%m-%dT%H:%M:%S")
                        endDateTime = datetime.datetime(int(completeData[0]), int(completeData[1]),
                                                        int(completeData[2]), int(endTime[0]), int(endTime[1]) + 1,
                                                        0).strftime("%Y-%m-%dT%H:%M:%S")

                        if (subject and subject != "x"):  # i there is a subject i add the event
                            listEventi.append(
                                {"Subject": subject, "StartDateTime": startDateTime, "EndDateTime": endDateTime})
...
def elementExists(event, google_list):
    for google_event in google_list:
        if (event["Subject"] == google_event["Subject"] and event["StartDateTime"] == google_event["StartDateTime"] and
                event["EndDateTime"] == google_event["EndDateTime"]):
            return True
    return False
...
def removeElementGoogleList(event_to_remove, new_google_list):
    for event_google in new_google_list:
        if (event_to_remove["Subject"] == event_google["Subject"] and event_to_remove["StartDateTime"] == event_google[
            "StartDateTime"] and event_to_remove["EndDateTime"] == event_google["EndDateTime"]):
...
def updateList(updated_list_events):
    ...
    print("ELEMENT TO new_google_list: [\n", new_google_list, "\n\n]")
    print("ELEMENT updated_list_events ADD: [\n", updated_list_events, "\n\n]")
