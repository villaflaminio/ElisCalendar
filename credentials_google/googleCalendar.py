...
def getConn():
    ...
    # time.
    if os.path.exists('credentials_google/token.pickle'):
        with open('credentials_google/token.pickle', 'rb') as token:
            creds = pickle.load(token)
    # If there are no (valid) credentials_google available, let the user log in.
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                'credentials_google/credentials_google.json', SCOPES)
            creds = flow.run_local_server(port=8080)
        # Save the credentials_google for the next run
        with open('credentials_google/token.pickle', 'wb') as token:
